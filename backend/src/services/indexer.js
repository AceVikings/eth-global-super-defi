...
const CONTRACT_ADDRESSES = {
  OPTIONS_TRADING:
    process.env.OPTIONS_TRADING_ADDRESS ||
    "0xcd9948d810c4e8c2144c4e2fb84786502e6bedc8",
  STABLE_COIN:
    process.env.STABLE_COIN_ADDRESS ||
    "0x43d109c41de6beab2e1d151d932bcc6318fa8f50",
  BITCOIN_TOKEN:
    process.env.BITCOIN_TOKEN_ADDRESS ||
    "0x70b0efc2b112d37cfeb2641cfde41b8677375935",
  WETH_TOKEN:
    process.env.WETH_TOKEN_ADDRESS ||
    "0x52e5d5ff769e71dfeead1a3fc5c440f87031a3e3",
  BTC_PRICE_FEED:
    process.env.BTC_PRICE_FEED_ADDRESS ||
    "0x2574b49a1ded38c9f239682769e3c3e708797c7a",
  ETH_PRICE_FEED:
    process.env.ETH_PRICE_FEED_ADDRESS ||
    "0x7d0c4127c937aaf59b0af8f686d63d602e27a777",
  TIME_ORACLE:
    process.env.TIME_ORACLE_ADDRESS ||
    "0x12aece39b96768dc9a776b1b3176b2bc21063314",
};
...
class OptionsIndexer {
      ...
      while (foundOptions < 100) {
        ...
        try {
          ...
          if (option.id > 0n) {
            ...
            foundOptions++;
            console.log(
              `📋 Indexed option ${optionId}: ${
                option.optionType === 0 ? "CALL" : "PUT"
              } ${this.formatPrice(option.strikePrice)}`
            );
          } else {
  ...
  getStats() {
    ...
    const purchased = options.filter(
      (o) => o.buyer !== "0x0000000000000000000000000000000000000000"
    ).length;
