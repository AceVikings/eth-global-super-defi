...
export const TimeManipulationComponent: React.FC<
  TimeManipulationComponentProps
> = ({ className = "" }) => {
  ...
  const setOracleTime = async (timestamp: number) => {
    ...
    try {
      ...
      const date = new Date(timestamp * 1000);
      setSuccess(
        `Oracle time updated to ${date.toLocaleString()} successfully!`
      );
  ...
  const increaseTime = async (seconds: number) => {
    ...
    try {
      ...
      setSuccess(
        `Oracle time increased by ${hours}h ${minutes}m successfully!`
      );
  ...
  return (
    <div className={className}>
      <div className="terminal-panel">
        <div className="mb-6">
          <h3
            className="text-2xl font-mono font-bold mb-2 uppercase tracking-wider"
          ...
          >
            MANIPULATE TIME FOR DEMO PURPOSES. THIS AFFECTS OPTION EXPIRY
            CALCULATIONS AND ALLOWS TESTING OF TIME-DEPENDENT FEATURES.
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="mb-5">
            <label
              className="block text-sm font-mono mb-3 uppercase tracking-wide"
              style={{ color: "var(--retro-amber)" }}
            >
              Quick Time Actions
            </label>
