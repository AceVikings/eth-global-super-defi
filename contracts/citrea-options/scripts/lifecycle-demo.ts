...
function loadLatestDeployment() {
  ...
  if (!fs.existsSync(deploymentPath)) {
    throw new Error(
      "❌ No deployment found. Please run deployment script first."
    );
  }
...
async function main() {
  ...
  console.log(
    `🏢 Network: ${deployment.network.name} (Chain ID: ${deployment.network.chainId})`
  );
  ...
  const stablecoin = await viem.getContractAt(
    "MockERC20",
    deployment.contracts.Stablecoin.address
  );
  const wbtc = await viem.getContractAt(
    "MockERC20",
    deployment.contracts.WBTC.address
  );
  const weth = await viem.getContractAt(
    "MockERC20",
    deployment.contracts.WETH.address
  );
  const layeredOptions = await viem.getContractAt(
    "CitreaLayeredOptionsTrading",
    deployment.contracts.LayeredOptions.address
  );
...
🎯 LayeredOptions: ${deployment.contracts.LayeredOptions.address}
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const usdcBalance = await stablecoin.read.balanceOf([
  ...
  for (let tokenId = 1n; tokenId <= 5n; tokenId++) {
    try {
      ...
      const assetName =
        baseAsset.toLowerCase() === wbtc.address.toLowerCase()
          ? "WBTC"
          : "WETH";
      ...
      console.log(
        `  Type: ${optionTypeName} ${isChild ? "(Child)" : "(Parent)"}`
      );
      console.log(`  Strike: $${Number(strikePrice) / 10 ** decimals}`);
      console.log(
        `  Premium: ${Number(premium) / (isChild ? 1e6 : 10 ** decimals)} ${
          isChild ? "USDC" : assetName
        }`
      );
      ...
      // Check ownership
      const owner1 = await layeredOptions.read.balanceOf([
        trader1.account.address,
        tokenId,
      ]);
      const owner2 = await layeredOptions.read.balanceOf([
        trader2.account.address,
        tokenId,
      ]);
      const owner3 = await layeredOptions.read.balanceOf([
        trader3.account.address,
        tokenId,
      ]);
  ...
  try {
    ...
    console.log(
      "   ✅ Correctly rejected: Non-owner cannot create child options"
    );
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    console.log(
      `  USDC: ${Number(usdcBalance) / 1e6} (${
        Number(usdcBalance) < 100000 * 1e6 ? "decreased" : "unchanged"
      })`
    );
    console.log(
      `  WBTC: ${Number(wbtcBalance) / 1e8} (${
        Number(wbtcBalance) < 10 * 1e8 ? "decreased" : "unchanged"
      })`
    );
    console.log(
      `  WETH: ${Number(wethBalance) / 1e18} (${
        Number(wethBalance) < 100 * 1e18 ? "decreased" : "unchanged"
      })`
    );
  }
...
${transactions
  .map(
    (tx, i) =>
      `${i + 1}. ${tx.trader}: ${tx.action} ${
        tx.strike ? `(${tx.strike})` : ""
      } - Premium: ${tx.premium || "N/A"}`
  )
  .join("\n")}
