...
describe("Child Options Enhanced Logic", function () {
  it("Should create child CALL with higher strike", async function () {
    ...
    await stablecoin.write.mint([user1.account.address, BigInt("1000000000000000000000")]);
    await stablecoin.write.approve([layeredOptions.address, BigInt("1000000000000000000000")], {
    ...
    assert.strictEqual(childOption[6].toLowerCase(), stablecoin.address.toLowerCase()); // premium token
    assert.strictEqual(childOption[4], 1n); // parent token ID

    console.log("✅ Child CALL option created successfully with higher strike!");
  });
  ...
  it("Should calculate and charge premium automatically", async function () {
    ...
    const balanceBefore = await stablecoin.read.balanceOf([user1.account.address]);
    ...
    const balanceAfter = await stablecoin.read.balanceOf([user1.account.address]);
    ...
    console.log(`✅ Premium calculated and charged correctly: ${expectedPremium.toString()} wei`);
  });
});